...
export default {
  ...
  mounted() {
    ...
    if (this.supplierData.name) this.nameInput.model = this.supplierData.name

    if (this.supplierData.email) this.emailInput.model = this.supplierData.email

    if (this.supplierData.phone) this.phoneInput.model = this.supplierData.phone
    if (this.supplierData.address)
      this.addressInput.model = this.supplierData.address
  },
  methods: {
    changeVal({ val, input }) {
      input.model = val
    ...
    async submit() {
      try {
        ...
        const { data } = await $axios.put(
          `${$config.apiURL}/suppliers/${this.supplierData._id}`,
          {
            name,
            phone,
            address,
