      ...
      <div class="body">
        <div class="tab">
          <div
            ...
            class="supplierData"
          >
            <div class="details" @click="edit(supplier)">
              <div class="supplier">
                {{ supplier.name }}
              </div>
...
export default {
  ...
  methods: {
    ...
    edit(user) {
      this.$root.$emit('supplier-edit', user)
    },
    async panggil() {
      const { data } = await this.$axios.get(`${this.$config.apiURL}/suppliers`)
      if (data.message === 'SUCCESS') this.suppliersList = data.suppliers
    },
    async deleteSupplier(user) {
      try {
        const { $axios, $config } = this
        const { data } = await $axios.delete(
          `${$config.apiURL}/suppliers/${user}`
        )
